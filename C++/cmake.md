...
在 CMake 中，PRIVATE、PUBLIC和INTERFACE是用于控制目标属性传播的关键字，主要影响头文件路径、编译选项和链接库的传递方式。理解它们的区别对于构建模块化、可复用的 CMake 项目至关重要。

关键字	作用范围	典型场景
PRIVATE	仅对当前目标生效，不传递给依赖它的其他目标。	仅内部使用的头文件、编译选项或库（如实现细节）。
PUBLIC	对当前目标生效，并传递给依赖它的其他目标（作为公共接口的一部分）。	公开 API 的头文件、必须由依赖者继承的编译选项或库（如第三方依赖）。
INTERFACE	仅传递给依赖它的其他目标，当前目标本身不使用。	纯接口库（如头文件库）或传递编译选项给依赖者（如 C++ 标准）。
