...
2.如果当前连续内存块不够长度，再找一个足够长的地方，分配一块新的内存q，并将p指向的内容copy到q，返回q。并将p所指向的内存空间删除。

### 1-1、calloc和malloc的区别
...
因此，realloc函数不会自动将新分配的内存初始化为0。如果需要将新分配的内存初始化为0，可以使用calloc函数来分配内存，它会将分配的内存初始化为0。
