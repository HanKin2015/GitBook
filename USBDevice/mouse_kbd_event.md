...
### 11-1、扫二维码输出中文
原理：https://www.php.cn/faq/657233.html
https://zhidao.baidu.com/question/145409662.html
把系统的内码（ASCII码）字库调出来了。按Alt+小键盘区的数字键

首先，汉字编码是计算机处理汉字信息的前提条件。它们通过将每个汉字与一个数字进行一一对应来实现。在计算机内，常用的汉字编码有国标码、区位码和机内码。这些编码方式能够准确表示汉字，并且在计算机系统中进行传输和处理。

国标码是一个四位十六进制数，它将一个汉字用两个字节表示，每个字节只有7位，与ASCII码相似。

如中文“一”的十进制编码为53947（GBK）或者19968（UTF-8），这里有个情况，当Windows系统是老版本的时候，微软输入法中文是使用的GBK编码，如1909，但是后面较新版本，如21H2，则使用的是UTF-8编码。这里可以设置回GBK编码：
右键微软输入法=》设置=》首选语言=》选项=》微软拼音=》常规=》兼容性=》使用以前版本的微软拼音输入法
老版本的Windows系统则没有这个设置，因此win7系统必是GBK编码格式。win10中这个新版和旧版输入法在显示上面有所区别，我还以为是被我设置弄变化了，还在拼命的修复。
win11：右键微软输入法=》设置=》常规=》兼容性=》使用以前版本的微软拼音输入法
...
内码是指操作系统内部的字符编码。早期操作系统的内码是与语言相关的.现在的Windows在内部统一使用Unicode，然后用代码页适应各种语言," 内码"的概念就比较模糊了。我们一般将缺省代码页指定的编码说成是内码。内码这个词汇，并没有什么官方的定义。代码页也只是微软的一种习惯叫法。作为程序 员，我们只要知道它们是什么东西，没有必要过多地考证这些名词。
...
有的HTML文件作者，特别是英文作者，认为世界上所有人都使用英文，在文件中不指定charset。如果他使用了0x80-0xff之间的字符，中文 Windows又按照缺省的GBK去解释，就会出现乱码。这时只要在这个html文件中加上指定charset的语句。
