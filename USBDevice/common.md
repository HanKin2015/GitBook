...
## 11、波特率
台式物理机（主板）自带的 RS-232 串口（DB9 接口），其最常见的最大波特率是 115200 bps。

主板自带的串口受 UART 芯片和驱动限制，绝大多数情况下 115200 就是稳定工作的上限。如果需要 230400、460800、921600 甚至 3 Mbps 的速率，通常要借助 PCIe 扩展卡 或 USB 转串口模块（如 FT232、CP2102 等芯片方案）。

| 芯片                        | 波形精度         | 稳定性                  | 价格                  | 推荐度    |
| ------------------------- | ------------ | -------------------- | ------------------- | ------ |
| **FT232 (FTDI)**          | ⭐ 最标准，字符严格连续 | 极高，工业级标杆             | 较贵 (~CH340 的 3~5 倍) | ⭐⭐⭐⭐⭐  |
| **CP2102 (Silicon Labs)** | 良好，字节间隔略大    | 高，丢包率仅 0.003%        | 中等                  | ⭐⭐⭐⭐   |
| **CH340**                 | 一般，间隔约 2 bit | 高波特率易丢包（~2.7%）       | 极便宜                 | ⭐⭐ 不推荐 |
| **PL2303**                | 一般           | Win10/11 驱动限制波特率，易蓝屏 | 便宜                  | ⭐ 不推荐  |
