...
注意：dpkg-deb中部分参数在dpkg命令中无法使用，如-R参数
...
-c	显示软件包中的文件列表
-e	将主控信息解压
-f	把控制信息打印到标准输出（DEBIAN/control文件）
-x	将软件包中的文件释放到指定目录下
...
-R	提取控制信息和存档的清单文件
-b	创建Debian软件包
注意这里是dpkg-deb命令，不是dpkg命令，部分参数在dpkg命令中无法使用
...
## 12、deb包签名
直接打包的deb包通过7zip解压后只有data.tar（2019版本7zip，但是2020版本测试发现解压正常），但是签名后的deb包解压后存在四个文件：control.tar.xz、data.tar.xz、debian-binary、sign。
发现并不是7zip软件版本问题，而是跟deb文件有关，另外一个linux-client-arm64-kylin.deb文件则使用最新版本2024也无法直接打开显示出其他文件，只能显示出data.tar文件。但通过鼠标右键->打开压缩包->\*，则会显示出control.tar.xz、data.tar.xz、_kylin.crt、_kylin.sig文件，但还是缺少一个debian-binary文件。通过ar命令则可以显示全所有文件。
ps：该deb文件大概率就是通过ar命令进行打包生成的。

正确处理方式还应该是ar命令。

使用dpkg-deb/dpkg命令是无法提取里面的debian-binary和sign文件，但是通过ar命令是可以的。
```
[root@ubuntu0006:/media/vdb/test] #ar -t linux-client-arm64-uos.deb
debian-binary
control.tar.xz
data.tar.xz
sign
...
data.tar.xz
sign
[root@ubuntu0006:/media/vdb/test] #ar -p linux-client-arm64-uos.deb debian-binary | cat -vte
2.0$
[root@ubuntu0006:/media/vdb/test] #ar t linux-client-arm64-kylin.deb 
debian-binary
control.tar.xz
data.tar.xz
_kylin.crt
...
### 12-1、ar命令
ar 命令是一个用于创建、修改和提取静态库（archive files）的工具，通常在 Unix 和 Linux 系统中使用。静态库是将多个目标文件（.o 文件）打包成一个文件，以便于链接和分发。

以下是 ar 命令的一些常用选项：
```
c：创建一个新的归档文件。
r：将文件添加到归档中，如果归档文件已存在，则替换其中的同名文件。
t：列出归档文件中的内容。
x：从归档中提取文件。
d：从归档中删除文件。
s：创建索引（符号表），以加快链接速度。
...
创建归档文件：
```
ar c libmylib.a file1.o file2.o file3.o
```
这条命令会创建一个名为 libmylib.a 的静态库，包含 file1.o、file2.o 和 file3.o。

添加文件到归档：
```
ar r libmylib.a file4.o
```
这条命令会将 file4.o 添加到 libmylib.a 中。

列出归档内容：
```
ar t libmylib.a
```
这条命令会列出 libmylib.a 中包含的所有文件。

提取文件：
```
ar x libmylib.a
```
这条命令会从 libmylib.a 中提取所有文件。

删除文件：
```
ar d libmylib.a file1.o
```
这条命令会从 libmylib.a 中删除 file1.o。

创建索引：
```
ar s libmylib.a
```
这条命令会为 libmylib.a 创建符号表，以加快链接速度。
