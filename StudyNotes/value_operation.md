...
## 13、FMEA与缺陷预防
FMEA（Failure Mode and Effects Analysis，故障模式及影响分析）与缺陷预防并不是同一个东西，但它们之间有密切的关系。

### 13-1、FMEA（故障模式及影响分析）
定义：FMEA 是一种系统化的方法，用于识别和评估产品或过程中的潜在故障模式及其对系统的影响。它帮助团队分析每种故障模式的严重性、发生概率和可检测性，从而确定优先级并采取相应的措施。
...
目的：FMEA 的主要目的是在产品设计或过程开发的早期阶段识别潜在问题，以便在问题发生之前采取预防措施，降低风险。

过程：
确定分析的对象（产品或过程）。
识别潜在的故障模式。
评估每个故障模式的影响。
计算风险优先级数（RPN），并确定优先处理的故障模式。
制定和实施改进措施。

### 13-2、缺陷预防
定义：缺陷预防是指在产品开发和生产过程中采取的措施，以防止缺陷的发生。它是一种更广泛的概念，涵盖了多种方法和工具，包括 FMEA、质量控制、过程改进等。

目的：缺陷预防的目标是通过系统化的方法和持续改进，减少或消除产品或过程中的缺陷，从而提高质量和客户满意度。

### 13-3、关系
FMEA 是缺陷预防的一部分：FMEA 是实现缺陷预防的一种有效工具。通过识别潜在的故障模式并采取相应的预防措施，FMEA 有助于减少缺陷的发生。
综合应用：在实际应用中，FMEA 可以与其他质量管理工具（如控制图、六西格玛、根本原因分析等）结合使用，以形成一个全面的缺陷预防体系。
