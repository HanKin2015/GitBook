...
## 粘包
在网络通信（尤其是基于 TCP 协议的通信）中，粘包指的是发送方发送的多个独立数据包，在接收方被合并成一个或多个 “混合包” 的现象。接收方无法直接从这些混合包中清晰地区分出发送方原本发送的每个独立数据包的边界，导致数据解析困难。

粘包是 TCP 流式传输的必然现象，本质是 “数据边界丢失”。解决粘包的关键是通过固定长度、分隔符、长度前缀或自定义协议等方式，主动为数据包添加可识别的边界，确保接收方能够正确拆分和解析数据。在实际开发中，基于 “长度前缀” 的方案因灵活性和可靠性被广泛采用。

## TCP超时机制如何设置的
CP 协议通过超时重传机制确保数据可靠传输，其核心是通过判断 “数据包发送后未收到确认的时间是否超过阈值” 来确认超时。具体过程涉及往返时间（RTT）计算、重传超时（RTO）动态调整和计时器管理三大环节。

超时的核心是 “多久未收到 ACK 算超时”，这个阈值称为重传超时时间（Retransmission Timeout，RTO）。
RTO 不是固定值，而是 TCP 根据网络往返时间（RTT） 动态计算的，目的是适应不同网络的延迟特性（如局域网延迟低，RTO 小；跨网延迟高，RTO 大）。
