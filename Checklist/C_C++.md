...
## 10、动态检查和静态检查区别
动态检查和静态检查是软件开发中常用的两种代码检查方法，它们的主要区别在于检查时机和检查方式。

静态检查是在编译时或者代码审查时对代码进行检查，不需要实际运行程序。静态检查通常使用一些工具或者手动审查代码来检查代码中的潜在问题，例如语法错误、类型错误、未定义变量等。静态检查可以帮助开发人员在编写代码时发现问题，提高代码质量和可靠性。

动态检查是在程序运行时对程序进行检查，需要实际运行程序。动态检查通常使用一些工具或者手动调试程序来检查程序中的问题，例如内存泄漏、空指针引用、数组越界等。动态检查可以帮助开发人员在程序运行时发现问题，提高程序的健壮性和可靠性。

总的来说，静态检查和动态检查都是重要的代码检查方法，它们可以相互补充，帮助开发人员发现和解决代码中的问题。静态检查可以在编写代码时发现问题，动态检查可以在程序运行时发现问题。

静态工具有：clang、address scanitizer、cppcheck
动态工具有：Valgrind
https://blog.csdn.net/radeyang/article/details/130315612

Jenkins本身并不是一个动态检查工具，它主要是一个自动化构建工具，可以用于自动化构建、测试和部署软件项目。Jenkins可以与其他动态检查工具集成，例如JUnit、Selenium等，以实现自动化测试和质量检查。

Jenkins提供了丰富的插件，可以支持各种不同的构建、测试和部署工具，也可以自定义插件来满足特定的需求。因此，Jenkins可以与各种不同的动态检查工具集成，例如静态代码分析工具、内存泄漏检测工具、性能测试工具等，以实现自动化的代码检查和质量控制。

总的来说，Jenkins本身并不是一个动态检查工具，但它可以与其他动态检查工具集成，以实现自动化的代码检查和质量控制。
...
std::list<std::unique_ptr<std::string>> m_policies; // 正确，不需要空格
std::list<std::unique_ptr<std::string> > m_policies; // 也正确，但不推荐
```

可能会遇到报错：error: a space is required between consecutive right angle brackets (use '> >')
这是因为在 C++11 之前，编译器在解析时可能会将 >> 视为位移操作符，而不是两个右尖括号。

- 在某些编译器或旧版本的 C++ 中，使用 >> 时需要在两个右尖括号之间添加空格。
- 现代 C++（C++11 及以后）通常不需要这样做，但为了兼容性，添加空格是一个好的做法。
如果你使用的是较新的编译器（如 GCC 4.3 及以上、Clang 3.0 及以上、MSVC 2010 及以上），通常不需要添加空格，但为了确保代码的兼容性，建议在 >> 之间加一个空格。
